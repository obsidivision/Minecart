...
function makeEngine() {
  ...
  function trackFromCode(code, origin, facing, opt) {
    ...
    if (opt && opt.boat) { T.loaded = true; T.startAt = boatFrac(opt.stopper || 'block', facing, opt.approach || 'behind'); }
  ...
  /* ---------- boat carts ----------
     ...
     (mcredstone/boat_prefab.py, boat_loader.py) hands the track a cart at rest on the start
     rail, at the fraction below, with these two changes to its physics:
  ...
  function searchTask(opt) {
    ...
    if (opt.boat) { T.loaded = true; T.startAt = boatFrac(opt.stopper || 'block', opt.facing, opt.approach || 'behind'); }
  ...
  function build(code, origin, facing, opt) {
    ...
    var loader = null;
    if (opt && opt.boat) loader = addLoader(put, at, g, f, opt.stopper || 'block', opt.approach || 'behind', ys, L, origin);
...
/* The 3D view of one track, on scene3d.js (the renderer the two 3D pages use).
   ...
   the blocks E.build() gives, the cart where it parks, and an outline where it lands once its
   parking rail is broken. For a boat cart it also shows the loading step, numbered: where to
   place the cart, the boat, the cart stopped against the stopper, the glass to break and the
   drop onto the start rail, with the loading run and the run itself as lines. */
function makeViewer3D() {
  var HW = 0.49000000953674316, HH = 0.699999988079071;        // the minecart's hitbox
  var el = document.createElement('div');
  el.className = 'gl3d';
  el.innerHTML = '<div class="gl-labels" aria-hidden="true"></div><div class="gl-bar"><span class="gl-hint">Drag to turn · scroll to zoom</span>' +
  ...
  return {
    ...
    load: function (blocks, cartPos, dropTo, boatPos, boat) {
      ...
      if (boatPos) S.boat(rel(boatPos));
      var labels = [];
      if (boat && boat.loader) {
        var L = boat.loader, mid = function (b, dy) { return rel([b[0] + 0.5, b[1] + (dy || 0), b[2] + 0.5]); };
        S.path('load', L.path.map(function (p) { return rel(p, 0.35); }), { key: 'run', xray: true });
        S.path('drop', [rel(L.stopAt, 0.35), rel(boat.landing, 0.35)], { key: 'run', xray: true });
        ...
        if (boat.path) S.path('run', boat.path.map(function (p) { return rel(p, 0.35); }), { key: 'sol', xray: true });
      ...
      if (cartPos) {
        ...
        if (boat && boat.loader) labels.push({ p: [p[0], p[1] + 1.0, p[2]], t: '5 Parks here' });
...
(function () {
  ...
  function syncCart() {
    var boat = choice('cart') === 'boat', st = $('stopper').value, ap = $('approach').value, facing = $('facing').value;
    $('stopBox').hidden = !boat;
    $('cartHint').textContent = boat
      ? 'The download brings a loading run: the cart picks the boat up, stops dead against the ' + STOP_WORD[st] +
        ', and you break the glass under it. It drops onto the start rail at fraction ' + E.boatFrac(st, facing, ap).toFixed(10) + ', at rest.'
  ...
  $('approach').addEventListener('change', syncCart);
  ...
  function readAll() {
    ...
    return { axis: choice('axis'), mode: tg.mode, target: tg.target, lo: tg.lo, hi: tg.hi, w: tg.w,
             ...
             sort: $('sort').value, boat: boat, stopper: boat ? $('stopper').value : null, approach: boat ? $('approach').value : null };
  }
  ...
  function workerMain() {
    ...
    self.onmessage = function (e) {
      var m = e.data;
      try {
        var r = E.searchTask({ boat: m.boat, stopper: m.stopper, approach: m.approach, facing: m.facing, alphabet: m.task.alphabet, depth: m.task.depth, origin: m.origin,
  ...
  function taskMsg(s, id) {
    ...
    return { id: id, task: t, facing: st.facing, origin: originFor(t.start, st.pos), axis: st.axis, target: st.target,
             lo: st.lo, hi: st.hi, sort: st.sort, keep: st.keep, boat: st.boat, stopper: st.stopper, approach: st.approach };
  }
  ...
  function leversOf(code, st) {
    var k = code + '|' + st.facing + '|' + (st.boat ? st.stopper + '|' + st.approach : '');
  ...
  function stKey(st) { return [st.pos.join(','), st.facing, st.axis, st.mode, st.target, st.lo, st.hi,
                               st.boat ? 'boat-' + st.stopper + '-' + st.approach : 'empty'].join('|'); }
  // what the engine needs to know about the cart: a boat aboard, and which stopper set it going
  function opts(st) { return st.boat ? { boat: true, stopper: st.stopper, approach: st.approach } : null; }
  ...
  function buildDetail(code, st) {
    ...
    if (!r.ok) {
      ...
      var gd = { blocks: b.blocks, cart: [r.x, r.y, r.z], drop: st.axis === 'y' ? null : drop, boat: b.loader ? b.loader.boat : null };
      if (b.loader) {                              // the loading step, drawn in the 3D view
        var tr = E.trackFromCode(code, origin, st.facing, o), c0 = E.spawnCart(tr.T), rp = E.run(code, origin, st.facing, true, 20000, o);
        gd.loader = b.loader; gd.landing = [c0.x, c0.y, c0.z]; gd.path = rp.ok ? rp.path : null;
      }
      glCache[key] = gd;
  ...
  function attachViewer(box) {
    ...
    v.load(d.blocks, d.cart, d.drop, d.boat, d.loader ? d : null);
  }
  function syncViewer() {
    var boxes = [];
  ...
  function download(code, st) {
    ...
    var desc = st.boat
      ? 'Found by the minecart alignment finder (created by 07km). Put a boat where the schematic marks it, place a plain minecart on the loading slope' +
        ...
        STOP_WORD[st.stopper] + ', then break the glass under it: the cart drops onto the start rail and ' + what +
  ...
  function verify(code, st, btn) {
    ...
    setTimeout(function () {
      var v = E.verify(code, { axis: st.axis, target: st.target, lo: st.lo, hi: st.hi, facing: st.facing,
                               boat: st.boat, stopper: st.stopper, approach: st.approach });
