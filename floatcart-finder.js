...
/*
  ...
  track height, and the straight fall when the parking rail is broken (dropY).
...
function makeEngine() {
  ...
  // slope start's rail is one block lower). level[k] is rail k's height offset.
  ...
  // is the conductor behind a standing start (a cart that rolls back to the start). A move
  // is cut short against it exactly as physics.py's collide() and entity_move() do.
  ...
  function collide(T, c, dx, dz) {
    ...
    var sx = T.X - T.f.du, sz = T.Z - T.f.dv, sy = T.Y;
    var s = [sx, sy, sz, sx + 1.0, sy + 1.0, sz + 1.0];
  ...
  function tick(T, c) {
    ...
    var oldY = getPosY(T, c.x, c.y, c.z);                   // track height before (R10)
    ...
    var hit = T.conductorBehind ? collide(T, c, dx, dz) : null;
  ...
  // The loading run in front of a boat track (mcredstone/boat_prefab.py): two slopes into a
  // flat run with the boat on it, ending on a rail straight above the start rail, with the
  // stopper in the next block. Breaking the glass under that rail drops the cart onto the
  // track. `put` and `at` come from build(), so everything lands in the same block list.
  function addLoader(put, at, f, stopper, facing, ys, L) {
    var GLASS = 'minecraft:white_stained_glass', CONCRETE = 'minecraft:white_concrete';
    var base = ys[0];                                                // levels here are from rail 0
    var lv = function (k) { return base + (k === BOAT.FIRST ? BOAT.LEVEL + 2 : k === BOAT.FIRST + 1 ? BOAT.LEVEL + 1 : BOAT.LEVEL); };
    var rails = [];
    for (var k = BOAT.FIRST; k <= BOAT.LAST; k++) {
      var shape = k <= BOAT.FIRST + 1 ? 'D' : 'F';
      put(at(k, lv(k)), 'minecraft:rail', { shape: SHAPE_NAMES[shapeOf(f, shape)], waterlogged: 'false' }, true);
      put(at(k, lv(k) - 1), GLASS, {}, true);
      rails.push({ k: k, pos: at(k, lv(k)), s: shape, kind: 'r' });
    }
    put(at(BOAT.FIRST - 1, base + BOAT.LEVEL + 2), GLASS, {}, true);        // cap: the first slope forms on it
    put(at(BOAT.FIRST - 1, base + BOAT.LEVEL + 3), 'minecraft:rail', { shape: SHAPE_NAMES[f.flat], waterlogged: 'false' }, true);
    var stopBlock;
    if (stopper === 'block') {
      stopBlock = at(BOAT.STOP_K, base + BOAT.LEVEL);
      put(stopBlock, CONCRETE, {}, true);
    } else {
      stopBlock = at(BOAT.STOP_K, base + BOAT.LEVEL);
      put(at(BOAT.STOP_K, base + BOAT.LEVEL, 1), CONCRETE, {}, true);       // the bud grows on this block
      put(stopBlock, STOPPER_BLOCK[stopper], { facing: BUD_SIDE[facing], waterlogged: 'false' }, true);
    }
    var boatRail = at(BOAT.BOAT_K, base + BOAT.LEVEL);
    return {
      rails: rails,
      start: at(BOAT.FIRST, base + BOAT.LEVEL + 2),                          // place the cart here
      boatRail: boatRail,
      boat: [boatRail[0] + 0.5, boatRail[1], boatRail[2] + 0.5],      // the boat's own spot
      stop: stopBlock,
      stopper: stopper,
      breakGlass: at(BOAT.LAST, base + BOAT.LEVEL - 1),                      // break this to drop the cart
      dropRail: at(BOAT.LAST, base + BOAT.LEVEL),
      level: BOAT.LEVEL
    };
  }
  ...
  function trackFromCode(code, origin, facing, opt) {
    ...
    if (opt && opt.boat) { T.loaded = true; T.startAt = boatFrac(opt.stopper || 'block', facing); }
    for (var i = 0; i < L.blocks.length; i++) {
      T.shape[i] = shapeOf(f, L.blocks[i].s); T.kind[i] = KINDS[L.blocks[i].k]; T.level[i] = ys[i];
    }
    T.n = L.blocks.length; T.conductorBehind = L.start === 'S';
  ...
  /* ---------- boat carts ----------
     ...
     (mcredstone/boat_prefab.py) hands the track a cart at rest on the start rail, at the
     fraction below, with these two changes to its physics:
       B1 every move is 0.75 x the speed, B2 it keeps 99.7% of its speed a tick, not 96%
       (OldMinecartBehavior, both keyed on isVehicle()).
       B3 the stop is the stopper's face less half a cart width: a plain block face, or a bud
       growing sideways out of a block beside the track, which starts 3/16 (medium, large,
       cluster) or 4/16 (small) inside its own block (C11). */
  var STOPPERS = { block: 0.0, bud: 3 / 16, small_bud: 4 / 16 };
  // the loading run, in steps along the track from the start rail and levels above it
  var BOAT = { LEVEL: 3, FIRST: -9, BOAT_K: -5, LAST: 0, STOP_K: 1 };
  var STOPPER_BLOCK = { bud: 'minecraft:medium_amethyst_bud', small_bud: 'minecraft:small_amethyst_bud' };
  var BUD_SIDE = { south: 'west', north: 'east', east: 'north', west: 'south' };
  function boatFrac(stopper, facing) {
    var inset = STOPPERS[stopper];
    if (inset === undefined) throw new Error('unknown stopper: ' + stopper);
    var f = FACINGS[facing], positive = (f.du + f.dv) > 0;
    var v = positive ? 1.0 + inset - HALF_W : 0.0 - inset + HALF_W;
  ...
  // at all (C4), so the cart stays where it is. Returns the y it ends at.
  var F_095 = Math.fround(0.95);
  function dropY(code, origin, facing, c, opt) {
  ...
  function searchTask(opt) {
    ...
    T.conductorBehind = opt.start === 'S';
    if (opt.boat) { T.loaded = true; T.startAt = boatFrac(opt.stopper || 'block', opt.facing); }
    ...
    function expand(k, snap, last, entry) {
      ...
      if (last.k !== 'p' && k >= minMid && pairOk(last, parkBlock)) {   // end the track here
        place(k + 1, parkBlock, entry);
        var c = park(snap, k);
        if (c) record(c, entry);
        T.n = k + 1;
      }
      if (k + 1 > opt.depth) return;
      for (var i = 0; i < alpha.length; i++) {                  // or add another rail
        ...
        place(k + 1, b, lvl);
        var c2 = advance(snap, k);
        if (c2) { tokens.push(b.t); expand(k + 1, c2, b, nxt); tokens.pop(); }
        T.n = k + 1;
      }
    }
  ...
  function build(code, origin, facing, opt) {
    ...
    var loader = null;
    if (opt && opt.boat) loader = addLoader(put, at, f, opt.stopper || 'block', facing, ys, L);
    ...
    return { blocks: list, ys: ys, L: L, levers: levers, rails: rails, caps: caps, sources: sources,
             conductor: conductor, poweredBy: poweredBy, side: lever, loader: loader };
  }
  ...
  function verify(code, o) {
    ...
    facings.forEach(function (fc) {
      PLACEMENTS.forEach(function (p) {
        res.runs++;
        var r = run(code, start === 'S' ? p : [p[0], p[1] + 1, p[2]], fc, false, 0, o.boat ? o : null);
        if (!r.ok) return;
        res.parked++;
  ...
  return {
    ...
    STOPPERS: STOPPERS, boatFrac: boatFrac, BOAT: BOAT
  };
}
...
/* The 3D view of one track, on scene3d.js (the renderer the two 3D pages use).
   ...
   the blocks E.build() gives, the cart where it stops, the boat for a boat cart, and an
   outline where the cart lands once its parking rail is broken. No run animation. */
function makeViewer3D() {
  var HW = 0.49000000953674316, HH = 0.699999988079071;        // the minecart's hitbox
  var el = document.createElement('div');
  el.className = 'gl3d';
  el.innerHTML = '<div class="gl-bar"><span class="gl-hint">Drag to turn · scroll to zoom</span>' +
    '<button type="button" class="btn sm glass-btn gl-reset">Reset view</button></div>';
  var S = null;
  try { S = makeScene3D(el, { keep: true, view: { az: 1.12, el: 0.34 }, pad: { bottom: 44 } }); } catch (e) { S = null; }
  if (!S) {
  ...
  return {
    el: el,
    ok: true,
    // blocks in world coordinates, drawn relative to their lowest corner (32-bit floats)
    load: function (blocks, cartPos, dropTo, boatPos) {
      var off = [Infinity, Infinity, Infinity];
      blocks.forEach(function (b) { off[0] = Math.min(off[0], b.x); off[1] = Math.min(off[1], b.y); off[2] = Math.min(off[2], b.z); });
      S.clear();
      S.blocks(blocks, { offset: off });
      if (boatPos) S.boat([boatPos[0] - off[0], boatPos[1] - off[1], boatPos[2] - off[2]]);
      if (cartPos) {
        var p = [cartPos[0] - off[0], cartPos[1] - off[1], cartPos[2] - off[2]];
        S.cart('sol').place(p).fit();
        if (dropTo !== null && dropTo !== undefined && dropTo !== cartPos[1]) {
          S.outline([p[0] - HW, dropTo - off[1], p[2] - HW], [2 * HW, HH, 2 * HW], 'sol', 0.75);
        }
      }
      S.grid(null, 1, true);
      S.resize();
...
(function () {
  ...
  function sizeHint() {
    var rails = choice('rails'), size = +choice('size'), s = SIZES[size], n = 0, secs = 0;
    jobsFor(rails, size).forEach(function (j) { var t = TOTALS[j.alphabet][j.depth]; n += t; secs += t / RATE[j.alphabet]; });
    ...
    $('sizeHint').textContent = len + ' About ' + fmtCount(n) + ' tracks: ' + fmtTime(secs) + ' on ' + cores + ' core' + (cores === 1 ? '' : 's') + '.' +
  ...
  /* ---------- boat carts ----------
     A cart that carries a boat cannot be placed by hand, so the page adds a fixed loading
     run in front of the track: the cart rolls down it, picks the boat up and stops dead
     against the stopper, always in the same spot. Breaking the glass under it drops it onto
     the start rail, at rest, at the fraction below. mcredstone/boat_prefab.py checks that in
     the full simulator; the search then runs with the two passenger factors. */
  function syncCart() {
    var boat = choice('cart') === 'boat', st = $('stopper').value, facing = $('facing').value;
    $('stopBox').hidden = !boat;
    $('cartHint').textContent = boat
      ? 'The download brings a loading run: the cart picks the boat up, stops against the ' +
        (st === 'block' ? 'block' : 'amethyst') + ', and you break the glass under it. It lands at fraction ' +
        E.boatFrac(st, facing).toFixed(10) + ', at rest.'
      : 'Placed by hand on the start rail, as in the maker schematics.';
  }
  each('input[name="cart"]', function (el) { el.addEventListener('change', syncCart); });
  $('stopper').addEventListener('change', syncCart);
  ...
  function readAll() {
    ...
    return { axis: choice('axis'), mode: tg.mode, target: tg.target, lo: tg.lo, hi: tg.hi, w: tg.w,
             ...
             sort: $('sort').value, boat: boat, stopper: boat ? $('stopper').value : null };
  }
  ...
  function workerMain() {
    ...
    self.onmessage = function (e) {
      var m = e.data;
      try {
        var r = E.searchTask({ boat: m.boat, stopper: m.stopper, facing: m.facing, alphabet: m.task.alphabet, depth: m.task.depth, origin: m.origin,
  ...
  function cmp(a, b) { return a.score - b.score || railCount(a.code) - railCount(b.code) || (a.code < b.code ? -1 : a.code > b.code ? 1 : 0); }
  function merge(top, list, keep) {
    var by = {};
    top.concat(list).forEach(function (r) { if (!by[r.code] || cmp(r, by[r.code]) < 0) by[r.code] = r; });
  ...
  function start() {
    ...
    jobsFor(st.rails, st.size).forEach(function (j) {
      total += TOTALS[j.alphabet][j.depth];
      E.makeTasks(j.alphabet, j.starts, j.depth, j.split).forEach(function (t) { t.minMid = j.minMid; tasks.push(t); });
    });
  ...
  function taskMsg(s, id) {
    var t = s.tasks[id], st = s.st;
    return { id: id, task: t, facing: st.facing, origin: originFor(t.start, st.pos), axis: st.axis, target: st.target,
             lo: st.lo, hi: st.hi, sort: st.sort, keep: st.keep, boat: st.boat, stopper: st.stopper };
  }
  ...
  function mainThreadStep(s) {
    ...
    while ((s.retry.length || s.next < s.tasks.length) && performance.now() - t0 < 40) {
      var m = taskMsg(s, s.retry.length ? s.retry.shift() : s.next++);
      var r = E.searchTask({ boat: m.boat, stopper: m.stopper, facing: m.facing, alphabet: m.task.alphabet, depth: m.task.depth, origin: m.origin,
        start: m.task.start, first: m.task.first, minMid: m.task.minMid, axis: m.axis, target: m.target, lo: m.lo, hi: m.hi,
        sort: m.sort, keep: m.keep });
      absorb(s, r.top, r.stats);
    }
  ...
  function renderRows(force) {
    ...
    s.top.forEach(function (r, i) {
      ...
      rows.push('<tr class="r' + (open ? ' open' : '') + '" data-code="' + esc(r.code) + '">' +
        '<td class="n">' + (i + 1) + '</td>' +
        '<td class="lay">' + tokensHTML(r.code) + '</td>' +
        '<td class="yf"><span class="mono">' + fmtFrac(axisValue(r, st.axis)) + '</span>' + badge + '</td>' +
        ...
        '<td class="num levers">' + leversOf(r.code, st) + '</td>' +
        '<td class="more"><button type="button" class="btn sm" aria-expanded="' + open + '">' + (open ? 'Hide' : 'Details') + '</button></td></tr>');
      if (open) rows.push('<tr class="detail"><td colspan="7">' + detailHTML(r.code, st) + '</td></tr>');
    });
    snapCurrent();
  ...
  var leverCache = {};
  function leversOf(code, st) {
    var k = code + '|' + st.facing + '|' + (st.boat ? st.stopper : '');
    if (!(k in leverCache)) leverCache[k] = E.build(code, originFor(startOf(code), st.pos), st.facing, opts(st)).levers;
    return leverCache[k];
  }
  ...
  /* ---------- one track in detail ---------- */
  var detailCache = {}, verifyCache = {};
  function stKey(st) { return [st.pos.join(','), st.facing, st.axis, st.mode, st.target, st.lo, st.hi,
                               st.boat ? 'boat-' + st.stopper : 'empty'].join('|'); }
  // what the engine needs to know about the cart: a boat aboard, and which stopper set it going
  function opts(st) { return st.boat ? { boat: true, stopper: st.stopper } : null; }
  function detailHTML(code, st) {
    var key = code + '|' + stKey(st);
    if (!detailCache[key]) detailCache[key] = buildDetail(code, st);
    var v = verifyCache[key];
    return detailCache[key].replace('@@VERIFY@@', '<p class="verify' + (v ? ' ' + v.cls : '') + '" data-verify="' + esc(code) + '"' + (v ? '>' + v.html : ' hidden>') + '</p>');
  }
  function buildDetail(code, st) {
    var key = code + '|' + stKey(st);
    var origin = originFor(startOf(code), st.pos);
    var o = opts(st);
    var r = E.run(code, origin, st.facing, false, 20000, o);
    var b = E.build(code, origin, st.facing, o);
    var h = '<div class="dv">', drop = null;
    if (!r.ok) {
      h += '<p class="check-sum">The cart does not park on this track: ' + esc(r.why) + '.</p>';
    } else {
      var v = axisValue(r, st.axis), d = E.circDist(v, st.target), inside = E.inRange(frac(v), st.lo, st.hi);
      drop = E.dropY(code, origin, st.facing, r, o);
      ...
      glCache[key] = { blocks: b.blocks, cart: [r.x, r.y, r.z], drop: st.axis === 'y' ? null : drop,
                       boat: b.loader ? b.loader.boat : null };
      h += '<div class="glbox" data-gl="' + esc(key) + '"></div>';
    }
    h += '<div class="dv-actions">' +
      '<button type="button" class="btn sm" data-act="copy" data-code="' + esc(code) + '">Copy layout code</button>' +
      '<button type="button" class="btn sm" data-act="lite" data-code="' + esc(code) + '">Download .litematic</button>' +
      (r.ok && !st.boat ? '<button type="button" class="btn sm" data-act="tester" data-code="' + esc(code) + '">Download with tester</button>' : '') +
      '<button type="button" class="btn sm" data-act="verify" data-code="' + esc(code) + '">Check at ' + (st.axis === 'y' ? 60 : 15) + ' positions</button>' +
      (st.boat ? '<span class="note">The automatic test only covers empty carts so far.</span>' : '') +
      '<span class="note" data-for="' + esc(code) + '"></span></div>';
    h += '@@VERIFY@@';
  ...
  function buildTableHTML(b, st) {
    ...
    var rows = b.rails.map(function (rl) {
      var notes = [];
      if (rl.k === 0) notes.push(b.loader ? 'the boat cart drops onto this rail' : 'place the cart here');
    ...
    var extra = '';
    if (b.loader) {
      var L = b.loader, st2 = L.stopper === 'small_bud' ? 'small amethyst bud' : L.stopper === 'bud' ? 'amethyst bud (medium, large or cluster)' : 'plain block';
      extra = '<p class="bnote">Loading run, all plain rails on glass, ' + b.loader.level + ' blocks above the start rail: it runs from ' +
        xyz(L.start) + ' (a slope down, place the minecart here) to ' + xyz(L.dropRail) + ', straight above the start rail. ' +
        'The boat goes on the rail at ' + xyz(L.boatRail) + '. The ' + st2 + ' sits at ' + xyz(L.stop) +
        (L.stopper === 'block' ? '' : ', grown on the block beside it') + '. Once the cart has stopped against it, break the glass at ' +
        xyz(L.breakGlass) + ': the rail above it pops off and the cart drops onto the start rail.</p>';
    }
  ...
  function attachViewer(box) {
    ...
    v.load(d.blocks, d.cart, d.drop, d.boat);
  }
  function syncViewer() {
    var boxes = [];
  ...
  function note(code, text) {
    each('.note[data-for]', function (el) { if (el.getAttribute('data-for') === code) el.textContent = text; });
  }
  function actionClick(e) {
    var btn = e.target.closest ? e.target.closest('button[data-act]') : null;
    if (!btn) return;
    e.stopPropagation();
    var code = btn.getAttribute('data-code'), act = btn.getAttribute('data-act');
    var st = btn.closest('#checkOut') && CHECK ? CHECK : S ? S.st : readAll();
    if (!st) return;
    if (act === 'copy') copy(code);
    else if (act === 'lite') download(code, st);
    else if (act === 'tester') downloadTester(code, st);
    else if (act === 'verify') verify(code, st, btn);
  }
  function copy(code) {
    function done() { note(code, 'Copied.'); }
    function fallback() {
      var ta = document.createElement('textarea');
      ta.value = code; ta.setAttribute('readonly', ''); ta.style.position = 'fixed'; ta.style.opacity = '0';
      document.body.appendChild(ta); ta.select();
      try { document.execCommand('copy'); done(); } catch (err) { note(code, 'Copy failed: select the code and copy it.'); }
      document.body.removeChild(ta);
    }
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(code).then(done, fallback);
    else fallback();
  }
  function download(code, st) {
    var origin = originFor(startOf(code), st.pos);
    var o = opts(st);
    ...
    var desc = st.boat
      ? 'Found by the minecart alignment finder (created by 07km). Put a boat where the schematic marks it, place a plain minecart on the loading slope' +
        (b.levers ? ' with the levers on' : '') + ', let it take the boat aboard and stop against the ' +
        (st.stopper === 'block' ? 'block' : 'amethyst') + ', then break the glass under it: the cart drops onto the start rail and ' + what +
        '. Layout code: ' + code + '. Minecraft Java 26.2.'
      : 'Found by the minecart alignment finder (created by 07km). Place a plain, empty minecart on the start rail' +
        (b.levers ? ' with the levers on' : '') + '; ' + what + '. Layout code: ' + code + '. Minecraft Java 26.2.';
    ...
    a.href = url; a.download = 'track-' + code.replace(/\s+/g, '-') + '.litematic';
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    setTimeout(function () { URL.revokeObjectURL(url); }, 4000);
    note(code, 'Saved. In the schematic, the start rail is ' + xyz(rel) + ' from its corner (' + enc.size.join(' × ') + ' blocks).');
  }
  ...
  function downloadTester(code, st) {
    var f;
    try { f = TS.file(code, st.pos, st.facing, st.axis, st.lo, st.hi, uid()); }
    catch (err) { note(code, 'No tester: ' + err.message + '.'); return; }
    ...
    note(code, 'Saved with an automatic test. Paste it in an open area of a creative world with commands allowed (on a server: op, and ' +
  ...
  function verify(code, st, btn) {
    btn.disabled = true;
    note(code, 'Checking…');
    setTimeout(function () {
      var v = E.verify(code, { axis: st.axis, target: st.target, lo: st.lo, hi: st.hi, facing: st.facing,
                               boat: st.boat, stopper: st.stopper });
      ...
      var txt = v.parked === 0 ? '<b>The cart parked in none of the ' + v.runs + ' runs.</b>' :
        ...
        (st.axis === 'y' ? '× 4 directions' : 'track running ' + st.facing) + '). ' + st.axis + ' fraction <span class="mono">' +
        v.min.toFixed(10) + '</span> to <span class="mono">' + v.max.toFixed(10) + '</span> (spread ' + fmtDist(v.max - v.min) + '). ' +
      ...
      verifyCache[code + '|' + stKey(st)] = { html: txt, cls: good ? 'good' : 'bad' };
      each('[data-verify]', function (el) {
        if (el.getAttribute('data-verify') !== code) return;
  ...
  function check() {
    ...
    snapCurrent();
    $('intro').hidden = true;
    out.innerHTML = '<p class="check-sum">' + tokensHTML(code) + '</p>' + detailHTML(code, st);
    syncViewer();
  }
